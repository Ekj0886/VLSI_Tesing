...
void CIRCUIT::GenerateCheckPointFaultList() {
    ...
    cout << "Generate check point stuck-at fault list" << endl;
    register unsigned i, j;
    GATEFUNC fun;
    GATEPTR fanout;
    FAULT *fptr;
    for(auto gptr : Netlist) {
        ...
        if(gptr->No_Fanout() > 1) {
            for(int i = 0; i < gptr->No_Fanout(); i++) {
                GATEPTR branch = gptr->Fanout(i);
                fptr = new FAULT(gptr, branch, S0);
                fptr->SetBranch(true);
                CP_Flist.push_front(fptr);
                fptr = new FAULT(gptr, branch, S1);
                fptr->SetBranch(true);
                CP_Flist.push_front(fptr);
            }
        }
...
void CIRCUIT::CheckPointOutput() {
    ...
    cout << "Flist size: " << Flist.size() << endl;
    cout << "CP_Flist  : " << CP_Flist.size() << endl;
    cout << "Using Checkpoint collapse: " << Percentage << " % faults" << endl;
}


void CIRCUIT::GenerateBridgeFaultList(string file) {

    ofstream outfile(file);
    vector<deque<GATE*>> Level(GetMaxLevel() + 1);
    BFAULT *BFptr;

    for(auto g : Netlist) Level[g->GetLevel()].push_back(g);
    
    for(unsigned int i = 0; i < GetMaxLevel(); i++) {
        while(Level[i].size() > 1) {
            GATE* g = Level[i].front();
            Level[i].pop_front();
            BFptr = new BFAULT(g, Level[i].front(), AND);
            B_Flist.push_back(BFptr);
            BFptr = new BFAULT(g, Level[i].front(), OR);
            B_Flist.push_back(BFptr);
        }
    }
    ...
    cout << "Dump Bridging Fault" << endl;
    for(auto f : B_Flist) {
        outfile << "(" << f->GetInputGate()->GetName() << ", " << f->GetOutputGate()->GetName() << ", ";
        if(f->GetBValue()) outfile << "OR)" << endl;
        else               outfile << "AND)" << endl;
    }
